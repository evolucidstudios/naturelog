...
const expRoot = path.join(process.cwd(), "public", "exp");
...
async function buildExplorerTiles(): Promise<ExplorerTile[]> {
  const entries = await safeReadDir(expRoot);
  const tiles: ExplorerTile[] = [];

  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    const absolutePath = path.join(expRoot, entry.name);
...
export default async function ExperimentalIndexPage() {
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-7 text-ink/70 sm:text-base">
                This page scans <code className="rounded bg-white/60 px-1.5 py-0.5 text-[0.9em]">public/exp</code> and
                lists any top-level folder with HTML inside it, plus any top-level HTML file.
              </p>
            </div>
            <Link
              href="/"
        ...
        {tiles.length === 0 ? (
          <section className="mt-8 rounded-[30px] border border-dashed border-bark/18 bg-white/72 px-6 py-10 text-center shadow-[0_18px_54px_rgba(31,59,83,0.08)]">
            ...
            <p className="mt-4 text-lg text-bark/74">
              Drop folders or <code className="rounded bg-white px-1.5 py-0.5 text-[0.9em]">.html</code> files into{" "}
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-bark/48">
                      {tile.kind === "folder" ? "Folder" : "HTML file"}
                    </p>
                    <h2 className="mt-3 text-2xl font-semibold text-bark">
                      {tile.label}
                    </h2>
                  </div>
